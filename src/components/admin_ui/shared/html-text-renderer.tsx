 ...
 */
function cleanEscapedHtml(html: string): string {
  return html
    .replace(/\u200B/g, '')
    .replace(/\[@([^\]]+)\]/g, '@$1')
...
export function HtmlTextRenderer({
  ...
  // Helper function to strip HTML tags and decode entities (SSR-safe: no `document` on server)
  const stripHtmlTags = (html: string): string => {
    const cleaned = cleanEscapedHtml(html);
